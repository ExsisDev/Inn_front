...
import "./ChallengeFinishedDetails.css";

class ChallengeFinishedDetails extends Component {
   ...
   componentDidMount() {
      this.getProposalFinishedByChallenge(this.props.location.state.idChallenge);
   }


   async getProposalFinishedByChallenge(idChallenge) {
      let url = `${process.env.REACT_APP_BACK_URL}/proposals/proposalFinished/${idChallenge}`;
      ...
      await axios.get(url, {
         ...
         if (result.data) {
            this.setState({
               ...
               finalComments: result.data[0].challenge.final_comment
            });
         }
      }).catch((error) => {
                        ...
                        <div className="formBox mb-5">
                           <Row className="m-0 d-flex justify-content-center">
                              <Col xs={9} className="challengeAssignedDetailsPadding">
                                 ...
                                 <Row className="mx-0 mt-3 text-left">
                                    <h6 className="trackAssigmentSubTitle mb-3">Comentarios finales: </h6>
                                    <p className="text-small ">{this.state.finalComments}</p>
                                 </Row>
                              </Col>
                           </Row>
                        </div>
